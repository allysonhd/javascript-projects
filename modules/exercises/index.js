  ...
  if (response.toLowerCase()==='y'){
    if (i===0){
      ...
      for (let j = 0; j<testTitles.length; j++){
        let avg = testTitles.averageForTest(j, scores); //Call 'averageForTest' here. Pass in j and scores as arguments.***
        console.log(`${testTitles[j]} test average = ${avg}%.`);
      }
    } else if (i===2){
      for (let j = 0; j<astronauts.length; j++){
        let avg = averages.averagesForStudent(j, scores); //Call 'averageForStudent' here. Pass in j and scores as arguments.
        console.log(`${astronauts[j]}'s test average = ${avg}%.`);
      }
    } else {
      let walker = astronauts.randomSelect(j); //Call 'randomSelect' to pick a spacewalker from the astronauts array.
